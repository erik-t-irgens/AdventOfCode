...
const findGoodPasswords = (input) => {
    let goodPass = 0

    for (let i = 0; i < input.length; i++) {
        let passwordArr = input[i].split(' ')
        let location1 = parseInt(passwordArr[0].split('-')[0])
        let location2 = parseInt(passwordArr[0].split('-')[1])
        let character = passwordArr[1][0]
        let password = passwordArr[2]

        if (password[location1 - 1] === character && password[location2 - 1] != character || password[location2 - 1] === character && password[location1 - 1] != character) {
            goodPass++
        }
